...
export default function GoogleSignInButton({ label = 'Continuar con Google', className }: { label?: string; className?: string }) {
  ...
  const isBgOverride = className?.includes('!bg-');

  return (
    <button type="button" onClick={handleLogin} disabled={loading}
      className={`inline-flex min-h-[52px] items-center justify-center gap-3 rounded-xl border px-6 text-[15px] font-semibold transition-all duration-150 active:scale-[0.98] disabled:opacity-60 ${
        isBgOverride
          ? 'border-transparent shadow-sm hover:brightness-110'
          : 'border-gray-300 bg-white text-[#0a0915] shadow-sm hover:bg-gray-50 hover:shadow-md'
      } ${className || ''}`}>
      {loading ? (
